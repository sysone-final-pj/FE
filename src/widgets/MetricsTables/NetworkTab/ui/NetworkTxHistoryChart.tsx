/********************************************************************************************
 * 📈 NetworkTxHistoryChart.tsx
 * REST API 기반 Network Tx (송신) 속도 추이 차트 (Time Range)
 ********************************************************************************************/
import {
  useState,
  useEffect,
...
export const NetworkTxHistoryChart = ({ selectedContainers }: Props) => {
    ...
    const fetchMetrics = async () => {
      ...
      try {
        ...
        validResults.forEach(({ metric }) => {
          metric.network.txBytesPerSec.forEach((point) => {
        ...
        {!timeFilter ? (
          <div className="h-full flex items-center justify-center">
            <p className="text-gray-400 text-sm">
              Select a time range to view Network Tx metrics
            </p>
          </div>
