/********************************************************************************************
 * 📈 NetworkHistoryChartForHistory.tsx
 * HistoryPage용 Network Rx/Tx 속도 추이 차트 (Time Range)
 ********************************************************************************************/
import {
  useState,
  useEffect,
...
export const NetworkHistoryChartForHistory = ({ containerId, containerName }: Props) => {
  ...
  useEffect(() => {
    ...
    const fetchMetrics = async () => {
      ...
      try {
        ...
        // 최대값 찾기 (단위 결정용)
        let maxValue = 0;
        [...metric.network.rxBytesPerSec, ...metric.network.txBytesPerSec].forEach((point) => {
        ...
        // Rx 데이터셋
        const rxDataset: ChartDataset = {
          label: `${containerName || `Container ${containerId}`} - Rx`,
          borderWidth: 2,
          fill: false,
          pointRadius: 0,
          pointHoverRadius: 4,
        ...
        // Tx 데이터셋
        const txDataset: ChartDataset = {
          label: `${containerName || `Container ${containerId}`} - Tx`,
          borderWidth: 2,
          fill: false,
          pointRadius: 0,
          pointHoverRadius: 4,
          pointHitRadius: 10,
          data: metric.network.txBytesPerSec.map((point) => {
            const converted = convertNetworkSpeedAuto(point.value);
            // 결정된 단위로 통일
            let value: number;
            if (unit === 'Kbps') {
        ...
        setDatasets([rxDataset, txDataset]);
      } catch (error) {
        void error;
        setDatasets([]);
      } finally {
  ...
  const options: ChartOptions<'line'> = useMemo(() => ({
    ...
    scales: {
      ...
      y: {
        ...
        title: {
          display: true,
          text: `Network Speed (${networkUnit})`,
        },
      },
    },
    plugins: {
        ...
        {!containerId ? (
          <div className="h-full flex items-center justify-center">
            <p className="text-gray-400 text-sm">
              Select a container to view Network metrics
            </p>
          </div>
        ) : !timeFilter ? (
          <div className="h-full flex items-center justify-center">
            <p className="text-gray-400 text-sm">
              Select a time range to view Network metrics
            </p>
          </div>
